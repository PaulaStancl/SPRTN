# WGS setup and test run on lobsang: sarek, oncoanalyser, tumourevo

This folder holds the one-time setup (01-04) and the HCC1395 test run. The runs on the real SPRTN data are in `../wgs/`.
...
└── projects/.../SPRTN/         this folder, copied to the server
    ├── scripts/wgs_test/       these scripts (setup + test run)
    ├── scripts/wgs/            real-data runs (RJALS)
...
Run from `scripts/wgs_test/`. Start steps 04 onwards inside `tmux`.
